...
function image_scan {
  PULL_IMAGES=(arminc/clair-db:latest arminc/clair-local-scan:v2.0.3 christiantragesser/clair-scanner)
  pull_images
  echo -e "${CYAN}---- Docker image CVE scan ----${NC}"
 
  docker network create $NETWORK || true
  docker run -d --name postgres --net $NETWORK arminc/clair-db:latest
  docker run --rm --net $NETWORK christiantragesser/clair-scanner /bin/sh -c "while ! timeout -t 1 sh -c 'nc -zv postgres 5432' &>/dev/null; do :; done"
